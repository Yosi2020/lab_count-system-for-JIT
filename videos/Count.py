...
def run():
    conf = {
        ...
        "display": True,
        # input video
        "input": join(dirname(__file__),"./videos/example_01.mp4"),
        # path the object detection model
        "model_path": join(dirname(__file__), "./mobilenet_ssd/MobileNetSSD_deploy.caffemodel"),
        "prototxt_path": join(dirname(__file__), "./mobilenet_ssd/MobileNetSSD_deploy.prototxt"),
        # output directory and csv file name
        "output": join(dirname(__file__),"./output/output_01.avi"),
        "csv_name": join(dirname(__file__), "./Logcs/Log.csv"),
    }
    ...
    while True:
        ...
        # the writer
        if conf["output"] is not None and writer is None:
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            writer = cv2.VideoWriter(conf["output"], fourcc, 30, (W, H), True)
        ...
        if config.Log:
            ...
            with open(conf['csv_name'], 'w', newline='') as myfile:
